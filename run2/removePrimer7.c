...
void usage(void) {
  ...
  fprintf(stderr, "                     for the forward primer (def. 0 [i.e. search will begin\n");
  fprintf(stderr, "                     at the first base of each read]). For example, specifying\n");
  fprintf(stderr, "                     %s -1,1 will allow the forward primer to match starting\n", FWDPOS);
  fprintf(stderr, "                     at positions -1, 0, or 1\n");
  fprintf(stderr, "  %s <int[,int]>  Position (or range of positions) at which to begin searching\n", REVPOS);
  fprintf(stderr, "                     for the reverse primer (def. 0 [i.e. search will begin\n");
  fprintf(stderr, "                     at the last base of each read])\n");
  fprintf(stderr, "  %s <int>        Mismatches to the forward primer to allow (def. 0)\n", MISALLOW);
  fprintf(stderr, "  %s <int>        Mismatches to the reverse primer to allow for full-length,\n", REVMIS);
  fprintf(stderr, "                     3' end checking (def. 0)\n");
  fprintf(stderr, "  %s <int>        Check also for internal matches of the reverse primer to the\n", REVLENGTH);
  fprintf(stderr, "                     read, using the specified length of the primer\n");
  fprintf(stderr, "  %s <int>        Mismatches to the reverse primer to allow for internal\n", REVLMIS);
  fprintf(stderr, "                     matching (def. 0)\n");
  fprintf(stderr, "  %s  <file>       Check also for minimal matches of the reverse primer to the\n", BEDFILE);
  ...
  fprintf(stderr, "  %s <int[,int]>  Position (or range of positions) at which to begin searching\n", BEDPOS);
  fprintf(stderr, "                     for the reverse primer based on expected length (def. 0)\n");
...
/*
printf("%s\n%s", line, p->fwd);
if (end) for (int i = st; i < end; i++) printf(" "); else printf("\t\t\t");
printf("%s\n", p->rev);
for (int i = 0; i < st; i++) printf(" ");
if (end) for (int i = st; i < end; i++) printf("%c", line[i]); else printf("not a match");
printf("\n");
while (!getchar()) ;
*/
...
/* FILE* openWrite()
 * Opens a file for writing, unless it already exists.
 */
...
void getParams(int argc, char** argv) {
  ...
  if (log != NULL) {
    ...
    fprintf(log, "Matches:\nPrimer\tFwd\tFwd and Rev(RC)\tRev\tRev and Fwd(RC)\n");
    for (Primer* p = primo; p != NULL; p = p->next)
