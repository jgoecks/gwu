...
sub usage {
  print q(Usage: perl checkAltMapping.pl  <infile1>  <infile2>  <infile3>  <infile4>  <genome>  <outfile>  <logfile>
  Required:
    ...
    <outfile>  Output file
  Optional:
    <logfile>  Output file that lists primer match judgments
);
  exit;
}
...
while ($line) {
  ...
  while ($line) {
    ...
    if ($div[2] ne $cut[0] || $pos < $cut[1] || $pos > $cut[2]) {
      ...
      my $fwdP; my $revP = "";
      if ($rc) {
        $fwdP = substr($chr{$div[2]}, $div[3] - 1 + $off + length $div[9], length $fiveP);
        $fwdP = revComp($fwdP);
        if ($both) {
          $revP = substr($chr{$div[2]}, $div[3] - 1 - length $threeP, length $threeP);
        }
      } else {
        $fwdP = substr($chr{$div[2]}, $div[3] - 1 - length $fiveP, length $fiveP);
        if ($both) {
          $revP = substr($chr{$div[2]}, $div[3] - 1 + $off + length $div[9], length $threeP);
          $revP = revComp($revP);
        }
      }
      $fwdP =~ tr/a-z/A-Z/;
      ...
      my $scoreF = scoreAlign($fwdP, $fiveP);
      my $scoreR = scoreAlign($revP, $threeP) if ($both);

      # determine result
      my $res = 1;
      $res = 0 if ($scoreF < 0.75 || ($both && $scoreR < 0.75));
